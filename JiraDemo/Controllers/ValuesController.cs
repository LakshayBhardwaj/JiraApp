        ...
        public JObject PostAllProducts(Filter filter)
        {
            ...
            for (int l = 0; l < uniqueProjectNames.Count(); l++)
            {
                ...
                for (var k = 0; k < projectNames.Count(); k++)
                {
                    ...
                    {
                        var assignee = "";
                        if (jresult["issues"][k]["fields"]["assignee"] == null || jresult["issues"][k]["fields"]["assignee"].ToString()=="")
                        {
                            assignee = "";
                        }
                        else
                        {
                            System.Diagnostics.Debug.WriteLine(jresult["issues"][k]["fields"]["assignee"]);
                            assignee = (string)jresult["issues"][k]["fields"]["assignee"]["name"];

                        }
                        Issue1.Add(new Issue { Name = (string)jresult["issues"][k]["fields"]["summary"], Id = (int)jresult["issues"][k]["id"], Type = (string)jresult["issues"][k]["fields"]["issuetype"]["name"], Status = (string)jresult["issues"][k]["fields"]["status"]["name"], Assignee = assignee});
                    }
                }
